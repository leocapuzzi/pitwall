...
function Countdown({ to, prefix }: { to: string; prefix: string }) {
  ...
  if (ms <= 0) return <b className="pw-seacount num">em andamento</b>
...
function Thumb({ t, className }: { t: CalThumb | null; className?: string }) {
  if (!t) return (
    <div className={'pw-calthumb miss ' + (className || '')}>
      <Icon n="flag" s={15} />
      <span>traçado no seu 1º stint</span>
    </div>
  )
  return (
    ...
    <div className="pw-season pw-glass2">
      <div className="pw-seahead">
        <div className="pw-seatitle">
          <span className="kicker">Temporada · {cal.season}</span>
        ...
        <div className="pw-seanext">
          {visao.proxCorrida ? (<>
            <span className="lbl">{visao.feitas ? 'Próxima corrida' : 'A temporada começa'}</span>
            <Countdown to={visao.proxCorrida.corrida} prefix="em" />
          </>) : <span className="lbl">temporada encerrada</span>}
        </div>
      </div>

      <div className="pw-searailwrap">
        <button className="pw-seaarrow l" aria-label="Semanas anteriores" onClick={() => desliza(-1)}><Icon n="chevR" s={14} /></button>
            ...
            return (
              <button key={w.w} data-w={w.w}
                ...
                <div className="pw-calhead">
                  ...
                  {st === 'live' && <span className="pw-callive">ESTA SEMANA</span>}
                  {real(w) && <span className="pw-calreal" title="Pista criada no PitWall — traçado real">✓</span>}
                </div>
        ...
        <button className="pw-seaarrow r" aria-label="Próximas semanas" onClick={() => desliza(1)}><Icon n="chevR" s={14} /></button>
      </div>
      ...
      <div className="pw-seatl" aria-hidden>
        ...
        <span className="pw-seacar" style={{ left: visao.prog * 100 + '%' }} title="Você está aqui na temporada" />
      </div>
        ...
        <div className="pw-calmodal" onClick={() => setAberta(null)}>
          <div className="pw-calsheet pw-glass2" onClick={e => e.stopPropagation()}>
            <div className="pw-calshead">
              <div>
                <span className="kicker">Semana {aberta.w} · {serie.nome}</span>
                <b className="pw-calsh">{aberta.pista}</b>
                <span className="pw-calscfg">{aberta.config || 'configuração única'}</span>
              </div>
              <button className="chip" onClick={() => setAberta(null)} aria-label="Fechar">✕</button>
            </div>
            <Thumb t={aberta.thumb ? cal.thumbs[aberta.thumb] : null} className="big" />
            <div className="pw-calfacts">
              <div><span className="lbl">Corrida (1x)</span><b className="num">{fmtCorrida(aberta.corrida)}</b></div>
              <div><span className="lbl">Semana</span><b className="num">{fmtDia(aberta.inicio)} → {fmtDia(new Date(dt(aberta.inicio).getTime() + 6 * 86400000))}</b></div>
              <div><span className="lbl">Clima</span><b className="num">{aberta.temp_c}°C · sem chuva</b></div>
              <div><span className="lbl">Largada</span><b className="num">{serie.largada} · {serie.duracao_min} min</b></div>
              <div><span className="lbl">Cadência</span><b>{serie.cadencia}</b></div>
              <div><span className="lbl">Licença</span><b>{serie.licenca}</b></div>
            </div>
            <div className={'pw-calstatus' + (real(aberta) ? ' ok' : '')}>
              {real(aberta)
                ? <>Pista criada no PitWall — o traçado acima é a centerline real da config.</>
                : <>Ainda sem telemetria desta config — rode 2+ voltas completas e o PitWall cria a pista (traçado, curvas e análise).</>}
            </div>
          </div>
        </div>
      )}
